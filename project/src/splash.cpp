...
void Splash() {
    ...
    short ttlW  = Assets::flymaplogo.width + Assets::jamlogo.width + LOGO_GAP;
    short flmpX = Globals::winCenterX - ttlW / 2;
    short flmpY = Globals::winCenterY - LIFT - Assets::flymaplogo.height / 2;

    short gJamX = flmpX + Assets::jamlogo.width + LOGO_GAP;
    ...
    while (!WindowShouldClose() && !delayIsOver(&timer, SPLASH_TIME)) {
        BeginDrawing();
        ClearBackground(BLACK);
        DrawTexture(Assets::flymaplogo, flmpX, flmpY, WHITE);
